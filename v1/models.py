...
class LSTM(nn.Module):
    def __init__(self, args,
        ...
        self.embed = nn.Embedding(self.input_dim, self.embed_dim, padding_idx=self.pad_idx)
        # pretrained  embedding
        if args.word_Embedding:
            self.embed.weight.data.copy_(args.pretrained_weight)
            self.dropout_embed = nn.Dropout(args.dropout_embed)  ###

        self.lstm = nn.LSTM(self.embed_dim, self.hid_dim, num_layers=self.num_layers, bias=True, bidirectional=self.bidirectional,
                              dropout=self.dropout)

        if init_weight:
            print("Initing lstm.parameters .......")
            for weight in self.lstm.parameters():
                utils.rnn_init(weight)

        self.hidden2hidden = nn.Linear(self.hid_dim * 2, self.embed_dim)

        self.logger.info(self.lstm)


    def forward(self, x, word_Embedding=False):
        x = self.embed(x)
        if word_Embedding:
        ...
        weighted = torch.transpose(lstm_out, 1, 2)

        #weighted = [batch size, emb dim, seq len]
        return weighted

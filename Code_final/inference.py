...
def evaluate(code_encoder, desc_encoder, code_pooler, desc_pooler, iterator, criterion):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(iterator, desc='Evaluating...'):
            ...
            # raw
            code_raw = code.cpu().numpy().tolist()
            code_raw = [code_denumericalizer.get(i) for i in code_raw[0]]
            code_raw = ' '.join(code_raw)

            # raw desc
            desc_raw = [desc_denumericalizer.get(i[0]) for i in desc.cpu().numpy().tolist()]
            if len(desc_raw) < 2:
                continue
            ...
            beam_search = BeamSearch(nmt_model=args.model, max_steps=30, beam_size=5, len_penality=0.7, lex_prob=0.1)

            gen_seq = beam_search.generate(encoded_code, desc_raw)
            scores = bleu_1.compute_bleu(desc_raw, gen_seq)

            res_scores.append(scores)
            print('bleu: {}'.format(scores))
...
if __name__ == '__main__':
    ...
    bleu_score = evaluate(code_encoder,
             desc_encoder,
             code_pooler,
             desc_pooler,
             test_iterator,
             criterion)
    score_test = sum(bleu_score) / len(bleu_score)
    print("ave scores: {}".format(score_test))
